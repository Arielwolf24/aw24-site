  ...
  return (
    <div style={{ position: 'fixed', inset: 0, display: 'flex', alignItems: 'center', justifyContent: 'center', zIndex: 2000, pointerEvents: 'auto' }}>
      <div style={{ textAlign: 'center', color: '#fff', opacity: visible ? 1 : 0, transition: 'opacity 360ms ease', padding: 24 }}>
        <h1 style={{ margin: 0, fontSize: '1.6rem' }}>/main is a little less empty for now, more coming soon!</h1>
        <div style={{ marginTop: 16, display: 'flex', flexDirection: 'column', gap: 8, alignItems: 'center' }}>
          ...
          <button onClick={() => window.dispatchEvent(new CustomEvent('enter-rgl'))} style={{ padding: '8px 14px' }}>RGL</button>
          <button onClick={() => navigate('/FurAffinityWarning')} style={{ padding: '8px 14px' }}>FurAffinity</button>
        </div>
      </div>
    </div>
  );
